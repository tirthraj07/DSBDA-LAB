...
// Hadoop Configuration Object -> Loads Hadoop settings like 'core-sites.xml', 'hdfs-sites.xml`, etc
import org.apache.hadoop.conf.Configuration;

// Used to easily parse command line arguments
import org.apache.hadoop.util.GenericOptionsParser;

// Hadoop Map Reduce Job Class -> create new map reduce instances
import org.apache.hadoop.mapreduce.Job;

// Get, Read and Write from HDFS
import org.apache.hadoop.fs.Path;
import org.apache.hadoop.mapreduce.lib.input.FileInputFormat;
import org.apache.hadoop.mapreduce.lib.output.FileOutputFormat;

// Datatypes -> Hadoop uses special type of datatype for serializing. Eg. int -> IntWritable, String -> Text
import org.apache.hadoop.io.IntWritable;
import org.apache.hadoop.io.Text;


// Exception thrown by map and reduce functions
import java.io.IOException;

// Used to tokenize string into list of words
...
Type | What it means | In this example
KEYIN | Input key | Object → file offset, e.g., 0L, 21L
VALUEIN | Input value | Text → a line from the file, like "hello world"
KEYOUT | Output key | Text → the word, like "hello"
VALUEOUT | Output value | IntWritable → always 1 (one word occurrence)
...
Part | What it means | In this example
KEYIN | The input key type | Text → the word (like "hello")
VALUEIN | The input values (grouped by key) | IntWritable → all the 1s from Mapper
KEYOUT | The output key type (what Reducer emits) | Text → the word again
VALUEOUT | The output value type (final count per word) | IntWritable → total count of that word
...
public class WordCount {
    public static class TokenizerMapper extends Mapper<Object, Text, Text, IntWritable>{
        ...
        // So, since we are always going to set the value of word to one, let us create a reusable object of IntWriter that has value 1
        ...
        // This is the main method that Hadoop calls automatically for every line of the input file.
    ...
    public static void main(String[] args) throws Exception{
        ...
        // It parses command-line arguments (args) like: hadoop jar wordcount.jar input.txt output
        // The getRemainingArgs() function strips out any Hadoop-specific options and leaves only: input path and output path
        ...
        job.setReducerClass(IntSumReducer.class);
            ...
            /*
            Optional -> setCombinerClass -> tell Hadoop to run a mini-reducer before the actual Reducer.
            Helps in network traffic
            In this program, it is safe to use it.
            Why? Notice how in reducer class, we doing sum += val.get() instead of just sum += 1
            The reason is, if a mini reducer runs before the actual reducer, it running mini reducer before, may give the input to reducer as
            "hello" : [1,2,1,4]
